...
本示例演示如何使用 RenderG SDK 完整提交一个 Clarisse iFX 渲染作业，涵盖从场景分析到下载结果的全流程。

**前置条件**

- 已创建 ``config.json``，包含 ``AUTH_KEY``、``CLUSTER_ID``、``ZONE_ID``、``RAM_LIMIT`` 等字段
- 已在 RenderG 客户端中创建好项目（``project_id``）和 Clarisse 渲染环境（``env_id``）

**流程说明**

1. **读取配置、初始化 API** —— 从 ``config.json`` 中读取认证信息，实例化 :class:`~renderg_api.core.RenderGAPI`
2. **分析场景** —— :class:`~analyze_clarisse.analyze.AnalyzeClarisse` 解析 ``.project`` 文件，
   收集依赖资产，生成 ``info.cfg`` 清单，并在服务器创建作业记录（得到 ``job_id``）
3. **设置渲染参数** —— :class:`~analyze_clarisse.param_check.ParamChecker` 通过
   ``set_render_layers`` 指定 Image 节点路径（如 ``project://render/cam2.color_sun_chr``）
   及帧范围，设置硬件配置
...
.. literalinclude:: ../../../example/submit_clarisse.py
   :language: python
   :linenos:
